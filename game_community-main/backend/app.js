...
app.use("/img/users", (req, res, next) => {
  // Reject requests where the path includes a full URL (external URL)
  if (req.path.startsWith("/http") || req.path.startsWith("/https")) {
    console.log(`Blocked invalid image request: ${req.originalUrl}`);
...
const fs = require("fs");
const defaultImagePath = path.join(__dirname, "public/img/users/default.jpg");
if (!fs.existsSync(defaultImagePath)) {
  // Instead of creating a PNG programmatically, create a simple placeholder text file to avoid dependency on pngjs
...
// Add detailed error logging middleware before globalErrorHandler
