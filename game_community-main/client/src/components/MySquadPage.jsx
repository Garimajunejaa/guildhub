...
export default function MySquadPage() {
  ...
  const handleCreateSquad = async () => {
    try {
      ...
      await axios.post("http://localhost:8080/api/v1/teams", newSquad, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setSnackbar({
        open: true,
  ...
  const handleAcceptInvite = async (invite) => {
    try {
      ...
      await axios.patch(
        `http://localhost:8080/api/v1/invites/${invite._id}/accept`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      const teamsRes = await axios.get("http://localhost:8080/api/v1/teams", {
        headers: { Authorization: `Bearer ${token}` },
      });
      let teams = [];
      ...
      if (teamToJoin) {
        ...
        await axios.patch(
          "http://localhost:8080/api/v1/teams/add-member",
  ...
  const handleDeclineInvite = async (inviteId) => {
    try {
      const token = localStorage.getItem("token");
      await axios.patch(
        `http://localhost:8080/api/v1/invites/${inviteId}/decline`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      setInvites((prevInvites) =>
                        ...
                        {squad.members.map((member, idx) => {
                          return (
                            <Avatar
                              ...
                              src={
                                member.photo
                                  ? `http://localhost:8080/img/users/${member.photo}`
                                  : undefined
                              }
                              alt={member.name}
                            >
