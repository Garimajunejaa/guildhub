...
export default function RecruitPlayersPage({ currentUser }) {
  ...
  useEffect(() => {
    socket.on("updateOnlineUsers", async (data) => {
      ...
      // try {
      //   await axios.post(
      //     "http://localhost:8080/api/v1/gv/update-my-gv",
      //     {},
      //     {
      //       headers: {
            ...
            allUsers.map(async (user) => {
              ...
              try {
                const res = await axios.get(
                  `http://localhost:8080/api/v1/gv/${user._id}`
                );
              ...
              // Import normalizeUserPhotoUrl at the top of the file
              // and use it here to normalize user photo URLs
              const { normalizeUserPhotoUrl } = await import("../../userPhotoUtils");

              return {
                ...user,
                isOnline,
                title: user.name,
                image: normalizeUserPhotoUrl(user.photo),
