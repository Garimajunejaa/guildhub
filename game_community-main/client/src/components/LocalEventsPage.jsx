...
export default function LocalEventsPage({ currentUser }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await axios.post("http://localhost:8080/api/v1/events", formData, {
        headers: {
  ...
  const handleProceedToPayment = async () => {
    try {
      await axios.patch(
        `http://localhost:8080/api/v1/events/add-member`,
      ...
      const { data } = await axios.get(
        `http://localhost:8080/api/v1/events/checkout-session/${selectedGame._id}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
                ...
                <Grid item xs={12} sm={4} key={i}>
                  ...
                  >
                    <CardMedia
                      component="img"
                      height="140"
                      image={`http://localhost:8080/img/events/${game.imageCover}`}
                      alt="Game cover"
                      onError={(e) => {
