...
import { normalizeUserPhotoUrl } from "../../userPhotoUtils";
...
const DEFAULT_USER_PHOTO = "/img/users/default.jpg";
...
export default function Auth() {
  ...
  const [ setUserPhoto] = useState(DEFAULT_USER_PHOTO);
  ...
  const handleLogin = async () => {
    try {
      ...
      // Set user photo or default
      setUserPhoto(data.user?.photo ? normalizeUserPhotoUrl(data.user.photo) : DEFAULT_USER_PHOTO);
      navigate("/");
    } catch (err) {
  ...
  const handleSteamLogin = () => {
    window.location.href = "http://localhost:8080/api/v1/users/auth/steam";
  };
